    ...
    try {
      await fetch('http://localhost:3001/agent/task-notify', {
        ...
        body: JSON.stringify({
          title: newTaskData.title,
          assignee: newTaskData.assignee,
          assigneePhone: assigneeMember?.phone,
          priority: newTaskData.priority,
          deadline: newTaskData.deadline,
          notifyWhatsapp: newTaskData.notifyWhatsapp,
          insertCalendar: true
        })
      });
    } catch (err) {
      console.error(err);
      // Fails silently for user UX or we can alert
      console.log("Notificação no backend falhou (servidor indisponível?)");
    }
                ...
                <div className="space-y-1 overflow-y-auto max-h-[50px] md:max-h-[80px] custom-scrollbar">
                  {dayTasks.map(t => (
                    <div
                      ...
                      className={`text-[9px] md:text-[10px] p-1 rounded cursor-pointer border-l-2 truncate transition-all ${t.completed ? 'opacity-40 line-through bg-white/5' : 'bg-white/5 hover:bg-white/10'
        ...
        <>
          <div className="flex gap-2 pb-2 overflow-x-auto custom-scrollbar">
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all whitespace-nowrap ${filter === 'All'
            ...
            {TEAM_MEMBERS.map((user) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all whitespace-nowrap ${filter === user.name
                    ? 'bg-lime-500 text-black'
                    : 'bg-white/5 text-slate-400 hover:text-white'
                  }`}
              >
                ...
                >
                  <div className="flex items-start gap-4 flex-1">
                    <button
                      ...
                      className={`w-6 h-6 mt-1 md:mt-0 rounded-full border-2 flex items-center justify-center transition-colors flex-shrink-0 ${task.completed ? 'border-green-500 bg-green-500 text-black' : 'border-slate-500 hover:border-lime-400'
                        }`}
                    >
                    ...
                    <div className={`flex-1 ${task.completed ? 'opacity-50 line-through' : ''}`}>
                      ...
                      <div className="flex items-center gap-3 text-xs text-slate-400 mt-2 md:mt-1 flex-wrap">
                        ...
                        <span className={`flex items-center gap-1 px-2 py-1 rounded text-[10px] font-bold ${task.priority === 'High' ? 'bg-red-500/20 text-red-400' :
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="glass-panel w-full max-w-md rounded-2xl p-8 border border-white/20 shadow-2xl animate-enter relative">
            ...
            <div className="flex gap-4 mt-8">
              ...
              <button onClick={handleAddTask} className="flex-1 py-3 bg-lime-500 hover:bg-lime-400 text-black font-bold rounded-xl shadow-lg shadow-lime-500/20 active:scale-95 min-w-[44px] min-h-[44px]">
                Salvar Tarefa
              </button>
            </div>
          </div>

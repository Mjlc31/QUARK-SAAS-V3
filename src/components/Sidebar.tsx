    ...
    <>
      ...
      <aside className={`fixed left-0 top-0 h-full w-72 bg-[#09090b] border-r border-white/5 flex flex-col z-50 transition-transform duration-300 lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-8 pb-8 flex justify-between items-center">
          ...
          <button onClick={onClose} className="lg:hidden w-11 h-11 flex items-center justify-center text-zinc-500 hover:text-white transition-colors min-w-[44px] min-h-[44px]">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="p-4 mt-auto border-t border-white/5">
          <div className="rounded-xl p-3 flex items-center gap-3 hover:bg-zinc-900/50 transition-colors cursor-pointer group">
            <div className="w-11 h-11 rounded-full bg-zinc-800 flex items-center justify-center border border-white/5 text-xs font-bold text-white relative">
              {user?.avatarInitials}
            ...
            <button onClick={logout} className="w-11 h-11 flex items-center justify-center text-zinc-600 hover:text-white transition-colors min-w-[44px] min-h-[44px]">
              <LogOut size={16} />
            </button>
          </div>
        </div>

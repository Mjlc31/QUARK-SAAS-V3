...
// ── CORES LUXURY ───────────────────────────────
const DARK = '#050505'; // Preto mais profundo
const SURFACE = '#0A0A0B';
const BORDER = '#ffffff15';
const MUTED = '#ffffff70';
const TEXT_H = '#ffffff';
...
const s = StyleSheet.create({
  ...
  coverOverlay: {
    ...
    backgroundColor: '#050505', opacity: 0.75,
  },
  ...
  h1: {
    fontSize: 64, color: '#ffffff', lineHeight: 1.05,
  ...
  clientName: {
    fontSize: 26, color: '#ffffff', letterSpacing: -0.5, marginBottom: 6
  },
  ...
  kpiCard: {
    flex: 1, backgroundColor: '#ffffff03',
    padding: '20 24',
    borderTopWidth: 1,
  },
  kpiLabel: {
  ...
  tableRowValue: { fontSize: 14, color: '#ffffff' },

  darkBanner: {
    backgroundColor: '#ffffff05', padding: '30 40',
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
  ...
  investPanel: {
    backgroundColor: '#ffffff03', padding: '40 50',
  ...
  investValue: { fontSize: 52, color: '#ffffff', letterSpacing: -1, lineHeight: 1, marginBottom: 30 },
  investOptions: { flexDirection: 'row', gap: 20, width: '100%' },
  investOption: {
    flex: 1, padding: '20 24',
    borderTopWidth: 1, backgroundColor: '#ffffff05',
  },
...
function PDFTechSpecs({ content: c, theme }: { content: TechSpecsContent; theme: ProposalTheme }) {
  ...
  const rows = [
    ...
    { icon: GaugeIcon, label: 'POTÊNCIA INSTALADA', value: `${totalPower.toFixed(2)} kWp`, accent: '#ffffff' },
      ...
      <View style={s.section}>
        ...
        <View style={s.kpiGrid}>
          {[
            ...
            { label: 'CONSUMO REF.', value: `${c.consumption} kWh/mês`, color: '#ffffff' },
            { label: 'ÁREA ESTIMADA', value: `${c.roofArea} m²`, color: '#ffffff' },
      ...
      <View style={s.section}>
        ...
        <View style={s.kpiFinGrid}>
          ...
          <View style={s.kpiFinCell}>
            ...
            <Text style={[s.kpiFinValue, { color: '#ffffff', fontFamily: fontSerifBold }]}>{fmtCurrency(cumSavings)}</Text>
          ...
          <View style={[s.kpiFinCell, { borderRightWidth: 0 }]}>
            ...
            <Text style={[s.kpiFinValue, { color: '#ffffff', fontFamily: fontSerifBold }]}>{roi.toFixed(0)}%</Text>
        ...
        {(tir > 0 || vpl !== 0) && (
          <View style={{ flexDirection: 'row', gap: 20, marginBottom: 24 }}>
            ...
            <View style={[s.kpiBox, { flex: 1, borderBottomWidth: 0, padding: 0 }]}>
              <Text style={[s.kpiBoxVal, { color: '#ffffff', fontSize: 24, fontFamily: fontSerifBold }]}>{fmtCurrency(vpl)}</Text>
        ...
        <View style={s.investPanel}>
          ...
          <View style={s.investOptions}>
            {[
              ...
              { label: `FINANCIADO ${c.installmentCount || 60}× DE`, value: fmtCurrency((c.finalPrice || 0) / (c.installmentCount || 60)), color: '#ffffff' },
            ].map(({ label, value, color }) => (
          ...
          {opts.map((opt: any, i: number) => {
            ...
            return (
              <View key={opt.id || i} style={{
                ...
                backgroundColor: highlighted ? `${pri}0D` : '#ffffff03',
                borderLeftWidth: 2, borderLeftColor: highlighted ? pri : 'transparent',
                borderWidth: 1, borderColor: highlighted ? `${pri}30` : BORDER
              }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 12, fontFamily: getSansFont(true), color: highlighted ? pri : '#ffffff', marginBottom: 8, letterSpacing: 1 }}>{opt.label}</Text>
                ...
                <View style={{ alignItems: 'flex-end', marginLeft: 20 }}>
                  ...
                  <Text style={{ fontSize: 24, fontFamily: fontSerifBold, color: highlighted ? pri : '#ffffff' }}>

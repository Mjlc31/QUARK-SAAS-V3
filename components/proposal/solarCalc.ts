...
export function calcSolar(input: SolarCalcInput): SolarCalcResult {
  ...
  // Saldo de energia após compensação
  const netConsumption = Math.max(0, monthlyConsumptionKwh - monthlyGenerationKwh);
  // Conta depois = max(custo disponibilidade, consumo_líquido * tarifa) + CIP
  const monthlyBillAfter = Math.max(custoDispo_R, netConsumption * tariffRate) + publicLighting;

  // Economia mensal = FC do Fio B (geração injetada) + economia na tarifa
  // Método correto: o que foi gerado economiza na fatura
  const energyCompensated = Math.min(monthlyGenerationKwh, monthlyConsumptionKwh);
